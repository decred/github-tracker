...
mkdir -p "${COCKROACHDB_DIR}/certs/clients/${USER_GHT}"
...
cp "${COCKROACHDB_DIR}/certs/ca.crt" \
  "${COCKROACHDB_DIR}/certs/clients/${USER_GHT}"

cockroach cert create-client ${USER_GHT} \
  --certs-dir="${COCKROACHDB_DIR}/certs/clients/${USER_GHT}" \
  --ca-key="${COCKROACHDB_DIR}/ca.key"
